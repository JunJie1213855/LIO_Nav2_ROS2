...
编辑脚本最后一行，把 `slam_toolbox` 替换为 `ros2 launch nav2_planner_bringup cartographer_mapping_launch.py use_sim_time:=false`。
...
ros2 service call /write_state cartographer_ros_msgs/srv/WriteState \
  '{filename: "src/planner/nav2_planner_bringup/map/airy_map.pbstream", include_unfinished_submaps: true}'
ros2 service call /finish_trajectory cartographer_ros_msgs/srv/FinishTrajectory '{trajectory_id: 0}'
ros2 run cartographer_ros cartographer_pbstream_to_ros_map \
  -pbstream_filename src/planner/nav2_planner_bringup/map/airy_map.pbstream \
  -map_filestem src/planner/nav2_planner_bringup/map/airy_map
```
...
ros2 launch nav2_planner_bringup cartographer_localization_launch.py \
    load_state_filename:=src/planner/nav2_planner_bringup/map/airy_map.pbstream use_sim_time:=false
```
...
```yaml
map_file_path: /ws/src/planner/nav2_planner_bringup/pcd/robo_map.pcd
```

改完后保存结果直接落在项目 `src/planner/nav2_planner_bringup/pcd/robo_map.pcd`（宿主可见），建图/导航（KISS-Matcher）直接引用。
...
docker exec lio_nav2 bash -c "
  ...
  ros2 service call /write_state cartographer_ros_msgs/srv/WriteState \
    '{filename: \"/ws/src/planner/nav2_planner_bringup/map/airy_map.pbstream\", include_unfinished_submaps: true}' && \
  ros2 service call /finish_trajectory cartographer_ros_msgs/srv/FinishTrajectory '{trajectory_id: 0}' && \
  ros2 run cartographer_ros cartographer_pbstream_to_ros_map \
    -pbstream_filename /ws/src/planner/nav2_planner_bringup/map/airy_map.pbstream \
    -map_filestem /ws/src/planner/nav2_planner_bringup/map/airy_map"
```
