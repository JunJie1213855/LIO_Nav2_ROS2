...
| ④ | 地形高程异常：RRT 扩展大量 "z>=1000 reject" | `terrain_voxle_elev_` 存点 **z**，体素内混入墙壁点 → `max-min Z ≥ 0.4` → `elev=1000`（不可通行） | `ground_ceiling_filter.py` 只保留 body z ∈ (-0.5, 0.35)：下界必须 < -0.3 才保留地面平面（body z≈-0.3），上界 0.35 去墙 | `/terrain_map_ext` 只含地面+低障碍 |
