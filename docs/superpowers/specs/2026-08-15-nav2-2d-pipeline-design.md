...
- 项目已有 2D 软件栈：`me_nav2_bringup` 含 Cartographer 配置、Nav2 配置、pointcloud_to_laserscan 桥。
...
| `src/me_nav2_bringup/config/cartographer_2d.lua` | Cartographer 2D 配置（use_odometry=true + use_imu_data=true） |
| `src/me_nav2_bringup/launch/cartographer_2d_launch.py` | cartographer_node + occupancy_grid_node |
| `src/me_nav2_bringup/launch/nav2_online_launch.py` | Nav2 在线导航（无 map_server/AMCL，复用 nav2_params.yaml） |
