...
{
  // [DEBUG] extension rejection counters (static, accumulate across calls)
  static int dbg_call = 0, dbg_minext = 0, dbg_z1000 = 0, dbg_dz = 0, dbg_line = 0, dbg_ok = 0;
  dbg_call++;
  ...
  while (!candidateFound)
  {
    ...
    if (count > 1000)
    {
      std::cout << "[RRT-DBG] plannerIterate gave up after 1000 samples."
                << " call=" << dbg_call << " ok=" << dbg_ok
                << " minext=" << dbg_minext << " z1000=" << dbg_z1000
                << " dz=" << dbg_dz << " line=" << dbg_line
                << " | root_z=" << root_[2] << " sample_z=" << newState[2] << std::endl;
  ...
  if (newState[2] >= 1000)  // the sampled position is above the untraversed area
  {
    dbg_z1000++;
    if (dbg_z1000 <= 3)
      std::cout << "[RRT-DBG] z>=1000 reject: sample=(" << newState[0] << "," << newState[1]
                << "," << newState[2] << ") parent_z=" << origin[2] << std::endl;
    return;
  }
  ...
  if (direction.norm() < params_.kMinextensionRange || direction[2] > params_.kMaxExtensionAlongZ)
  {
    dbg_dz++;
    if (dbg_dz <= 3)
      std::cout << "[RRT-DBG] dz/minext reject: parent_z=" << origin[2]
                << " new_z=" << newState[2] << " dz=" << direction[2]
                << " kMaxExtZ=" << params_.kMaxExtensionAlongZ << std::endl;
    return;
  }
  ...
  else
  {
    ...
    {  // connection is free
      ...
      dbg_ok++;
      if (dbg_ok <= 5)
        std::cout << "[RRT-DBG] NODE OK #" << dbg_ok << " z=" << newState[2]
                  << " gain=" << newNode->gain_ << " parent_z=" << newParent->state_[2] << std::endl;
    ...
    else
    {
      dbg_line++;
      if (dbg_line <= 3)
        std::cout << "[RRT-DBG] line/grid reject: from=(" << origin[0] << "," << origin[1] << "," << origin[2]
                  << ") to=(" << newState[0] << "," << newState[1] << "," << newState[2] << ")" << std::endl;
    }
  }
}
