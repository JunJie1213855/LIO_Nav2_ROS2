...
class FarLocalPlanner(Node):
    ...
    def cloud_cb(self, msg: PointCloud2):
        ...
        if n <= 0:
            return
        xs = np.frombuffer(msg.data, dtype=np.float32, count=n, offset=xo)
        ys = np.frombuffer(msg.data, dtype=np.float32, count=n, offset=yo)
        if zo is not None:
            zs = np.frombuffer(msg.data, dtype=np.float32, count=n, offset=zo)
        else:
