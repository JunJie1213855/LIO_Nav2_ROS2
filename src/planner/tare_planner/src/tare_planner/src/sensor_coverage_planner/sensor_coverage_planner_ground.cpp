...
void SensorCoveragePlanner3D::UpdateGlobalRepresentation() {
  ...
  pointcloud_manager_neighbor_cells_origin_point.header.frame_id = "odom";
...
void SensorCoveragePlanner3D::PublishGlobalPlanningVisualization(
  ...
  global_path_full.header.frame_id = "odom";
  ...
  global_path_trim.header.frame_id = "odom";
...
void SensorCoveragePlanner3D::PublishLocalPlanningVisualization(
  ...
  local_tsp_path.header.frame_id = "odom";
...
void SensorCoveragePlanner3D::execute() {
  ...
  execute_count++;
  if (execute_count <= 5) {
    RCLCPP_INFO(this->get_logger(), "[DEBUG] execute() called #%d, initialized_=%d, keypose_update=%d",
                execute_count, initialized_, keypose_cloud_update_);
  }
  ...
  if (!initialized_) {
    ...
    if (start_time_ == 0.0) {
      static int zero_time_warn_count = 0;
      if (zero_time_warn_count++ % 5 == 0) {
        RCLCPP_WARN(this->get_logger(),
                    "Start time is zero, waiting for a valid time source (/clock). "
                    "Is use_sim_time set correctly and is /clock being published?");
      }
      return;  // keep initialized_=false, retry on next tick
    }
    SendInitialWaypoint();
