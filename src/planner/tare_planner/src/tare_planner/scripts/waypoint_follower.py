...
class WaypointFollower(Node):
    ...
    def cloud_callback(self, msg: PointCloud2):
        ...
        for i in range(msg.width):
            ...
            # 分扇区记录最小距离（ly > 0 为机器人左侧）
            if ly < -half / 3.0:
                self.obs_right = min(self.obs_right, lx)   # 右侧
            elif ly > half / 3.0:
                self.obs_left = min(self.obs_left, lx)    # 左侧
            else:
    ...
    def control_loop(self):
        ...
        if self.obs_center < self.stop_dist:
            # 正前方被挡死：朝开阔侧硬转（先转开，不前进）
            desired_rel = (1.0 if open_left else -1.0) * 1.2
        elif nearest_obs < self.slow_dist:
            # 近处有障碍：若目标在开阔侧就朝目标走，否则朝开阔侧绕行
            goal_on_open = (open_left and goal_rel > 0.0) or (
                not open_left and goal_rel < 0.0)
            if goal_on_open:
                desired_rel = goal_rel
            else:
                desired_rel = (1.0 if open_left else -1.0) * 0.8
        else:
            desired_rel = goal_rel
