...
# Pane 2: KISS + GICP 重定位 (下左)
tmux split-window -v -t "$SESSION:nav.0"
tmux send-keys -t "$SESSION:nav.2" \
  "source install/setup.bash && ros2 launch global_relocalization_kiss_matcher global_kiss_matcher_relocalization_launch.py" C-m
tmux select-pane -t "$SESSION:nav.2" -T "KISS+GICP"
