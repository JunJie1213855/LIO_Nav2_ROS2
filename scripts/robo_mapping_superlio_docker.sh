#!/usr/bin/env bash
# 容器内 RoboSense Airy 数据集建图 — Super-LIO 版 (tmux)
# 用法:
#   docker exec -it lio_nav2 /ws/scripts/robo_mapping_superlio_docker.sh [bag路径]
#   docker exec -it lio_nav2 tmux attach -t robo_mapping_superlio
#   docker exec lio_nav2 tmux kill-session -t robo_mapping_superlio

SCRIPT_DIR="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)"
WORKSPACE_ROOT="$(dirname -- "$SCRIPT_DIR")"

SESSION=robo_mapping_superlio
...
# lio_interface — 用 superlio 模式（话题: /lio/odom → /lio/cloud_world）
...
echo "  1. Super-LIO 发布 /lio/odom + /lio/cloud_world"
echo "  2. lio_interface 用 lio_type:=superlio 自动匹配话题"
echo "  3. Super-LIO 自带 RViz (super_lio/rviz/lio.rviz)"
echo ""
echo "保存地图:"
